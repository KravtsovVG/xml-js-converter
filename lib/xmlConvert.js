    ...
    parser.onopentag = function (node) {
        ...
        if (thisParseState.$arrayElement) {
            ...
            thisState.storeChild = function (node, value) {
                ...
                if (typeof thisParseState.$arrayElement === 'string' && typeof value === 'object') {
                    value[thisParseState.$arrayElement] = node.name;
...
internals.objectToXml = function (builder, schema, jsObject) {
    ...
    if (typeof schema.$arrayElement === 'string' && jsObject[schema.$arrayElement]) {
        builder = builder.ele(jsObject[schema.$arrayElement]);
        objectSchema = schema[jsObject[schema.$arrayElement]] || {}
    }

    var contentName = objectSchema.$content === false ? false : (objectSchema.$content || '$content');

    var keys = Object.keys(jsObject);
    for(var index = 0; index < keys.length; ++index) {

        var schemaForKey = objectSchema[keys[index]] || {};
        ...
        if (keyIsContent) {
            ...
            internals.convertToXml(builder.ele(keys[index]), schemaForKey, jsObject[keys[index]]);
        } else if (typeof schema.$arrayElement !== 'string' || keys[index] !== schema.$arrayElement) {
